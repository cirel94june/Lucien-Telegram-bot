...
def _hub_get_context_network(user_message, recent_messages=None, chat_id="", chat_type=""):
...
def _extract_identity_relationship(text, replied_is_bot):
    ...
    if replied_is_bot:
        direct = re.fullmatch(
            rf"(?:这是|这个{bot_words}?是|它是|他是|她是)\s*(.{{1,32}}?)\s*的{bot_words}",
            body,
            flags=re.IGNORECASE,
        )
        if direct:
        ...
        named = re.fullmatch(
            rf"(.{{1,16}}?)\s*是\s*(.{{1,32}}?)\s*的{bot_words}",
            body,
            flags=re.IGNORECASE,
        )
        if named:
    ...
    reverse = re.fullmatch(
        rf"(.{{1,16}}?)\s*是\s*(?:他|她|这个人|这位)的{bot_words}",
        body,
        flags=re.IGNORECASE,
    )
    if reverse:
...
def _resolve_identity_reference(chat_id, reference, expected_is_bot):
    ...
    for uid, item in aliases.items():
        ...
        if bool(metadata.get("is_bot")) != bool(expected_is_bot):
            continue
        values = {
            str(metadata.get("alias") or "").strip().lower(),
            str(metadata.get("display_name") or "").strip().lower(),
...
def learn_identity_relationship(chat_id, bot_id, human_id, learned_by="", bot_alias=""):
    ...
    bot_item.update({
        "linked_human_id": human_uid,
        "relationship_updated_by": str(learned_by or ""),
        "relationship_updated_at": int(time.time()),
...
def describe_message_identity(chat_id, msg):
    ...
    lines = [
        ...
        f"内部身份：{_stable_sender_id(target_id, target_name, target_is_bot, chat_id)}",
        f"已记录昵称：{item.get('alias') or '无'}",
    ]
    linked_human_id = str(item.get("linked_human_id") or "")
    if linked_human_id:
            ...
            try:
                ...
                if text and str(text).strip():
                    ...
                    if "gemini" in str(model).lower():
                        ...
                        cot_text = _repair_model_mojibake(str(cot_text))
...
def process_message_background(text, chat_id, sender_name, msg_date=None,
                                ...
                                chat_type="", reply_reason="", sender_id="",
                                sender_is_bot=False, reply_to_message_id=None,
                                thread_id=None):
    try:
        tz = ZoneInfo(TIMEZONE)
